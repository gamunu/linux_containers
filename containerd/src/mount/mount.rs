...
impl Mount {
    ...
    pub fn mount(&self, target: &str) -> Result<(), String> {
        ...
        // propagation types.
        let ptypes: MsFlags = libc::MS_SHARED
            | libc::MS_PRIVATE
            | libc::MS_SLAVE
            | libc::MS_UNBINDABLE;
        ...
        if (flags & libc::MS_REMOUNT).bits() == 0 || data != "" {
            // Initial call applying all non-propagation flags for mount
            // or remount with changed data
            let source = self.source;
            if losetup {
                //setup loop hear
                let read_only: bool = oflags&unix.MS_RDONLY == unix.MS_RDONLY;
                losetup::setup_loop(read_only, true)
            }
        }
    ...
    fn parse_mount_options(&self, options: &Vec<String>) -> (MsFlags, String, bool) {
        ...
        let ms_flag: MsFlags = MsFlags::from_bits(flag).unwrap();

        (ms_flag, data.join(","), losetup)
    }
}
